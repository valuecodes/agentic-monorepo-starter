...
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
  # Mount at the real absolute path, not /repo: in a git worktree `.git` is a
  # file pointing at an absolute path inside the main checkout's git dir, which
  # therefore has to be mounted at its own path too.
  git_common_dir=$(cd "$repo_root" && cd "$(git rev-parse --git-common-dir)" && pwd)
  set -- -v "$repo_root:$repo_root" \
